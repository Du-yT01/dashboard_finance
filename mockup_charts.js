...
function initCharts(pageId) {
    switch (pageId) {
        ...
        case 'page-home': initFlowCharts(); break;
    }
}
...
// ── FLOW CHARTS (Luồng đọc báo cáo) ──────────────────────────────────────
function initFlowCharts() {
    // Helper: init only once
    const initOnce = (id, optFn) => {
        const el = document.getElementById(id);
        if (!el) return;
        if (echarts.getInstanceByDom(el)) return;
        echarts.init(el).setOption(optFn());
    };

    // 1. Gauge: Hệ số thanh toán hiện hành  — 1.15 (sát ngưỡng 1.1)
    initOnce('rf-gauge-current', () => ({
        animation: false,
        hoverLayerThreshold: -1,
        series: [{
            type: 'gauge',
            ...
            min: 0, max: 3, splitNumber: 5,
            axisLine: { lineStyle: { width: 28, color: [[0.37, colors.danger], [0.5, colors.warning], [1, colors.success]] } },
            pointer: { length: '50%', width: 5 },
            axisLabel: { distance: -38, fontSize: 10 },
            detail: { show: true, fontSize: 22, fontWeight: 'bold', offsetCenter: [0, '28%'], formatter: '{value}' },
            data: [{ value: 1.15, name: '' }],
            title: { show: false }
        }]
    }));

    // 2. Gauge: Hệ số Tiền mặt
    const rfGaugeOpt = (val, max) => ({
        animation: false,
        hoverLayerThreshold: -1,
        series: [{
            type: 'gauge',
            ...
            min: 0, max,
            splitNumber: 4,
            axisLine: { lineStyle: { width: 22, color: [[0.4, colors.danger], [0.7, colors.warning], [1, colors.success]] } },
            axisTick: { show: false },
            splitLine: { length: 8, lineStyle: { color: '#9ca3af', width: 1.5 } },
            axisLabel: { distance: -36, fontSize: 10, color: '#6b7280', formatter: v => v.toFixed(1) },
            pointer: { length: '52%', width: 4 },
            detail: { show: true, fontSize: 18, fontWeight: 'bold', offsetCenter: [0, '30%'], formatter: '{value}' },
            data: [{ value: val }],
            title: { show: false }
        }]
    });
    // Tiền mặt: 0.12 (rất thấp - đỏ sâu) | Nhanh: 1.05 (vừa đủ nhờ phải thu)
    initOnce('rf-gauge-cash',  () => rfGaugeOpt(0.12, 1));
    initOnce('rf-gauge-quick', () => rfGaugeOpt(1.05, 2));

    // 3. Waterfall: DT 8.5 tỷ → LN chỉ còn 0.2 tỷ (giảm mạnh so CK)
    initOnce('rf-waterfall', () => ({
        animation: false,
        hoverLayerThreshold: -1,
        tooltip: { trigger: 'axis' },
        grid: { containLabel: true, left: '2%', right: '6%', bottom: '8%', top: '14%' },
        xAxis: { type: 'category', data: ['Doanh thu', 'Giá vốn', 'LN Gộp', 'CP QLDN', 'CP TC', 'LN Thuần'],
            axisLabel: { fontSize: 11 } },
        yAxis: { type: 'value', axisLabel: { formatter: '{value} tỷ', fontSize: 10 } },
        series: [{
            type: 'bar', barWidth: '50%',
            data: [
                { value: 8.5,  itemStyle: { color: colors.primary } },
                { value: -6.5, itemStyle: { color: colors.danger } },
                { value: 2.0,  itemStyle: { color: colors.info } },
                { value: -1.3, itemStyle: { color: colors.warning } },
                { value: -0.5, itemStyle: { color: colors.pink } },
                { value: 0.2,  itemStyle: { color: colors.success } }
            ],
            label: { show: true, position: 'top', fontSize: 11, formatter: '{c} tỷ' }
        }]
    }));

    // 4. Top Expenses — % tăng cao để thể hiện chi phí mất kiểm soát
    const expData = [
        { name: 'Thuê sà lan',       value: 2200, change: '+18%' },
        { name: 'Thuê xe đầu kéo',   value: 1800, change: '+12%' },
        { name: 'Nhiên liệu',         value: 1200, change: '+22%' },
        { name: 'Nhân công vận hành', value: 850,  change: '+18%' },
        { name: 'Lương quản lý',      value: 650,  change: '+5%'  },
        { name: 'Khấu hao',           value: 500,  change: '0%'   },
        { name: 'Lãi vay',            value: 420,  change: '+5%'  },
        { name: 'Điện nước',          value: 320,  change: '+8%'  },
        { name: 'Văn phòng phẩm',     value: 280,  change: '+2%'  },
        { name: 'Bảo trì máy móc',    value: 230,  change: '+12%' }
    ];
    initOnce('rf-top-expenses', () => ({
        animation: false,
        hoverLayerThreshold: -1,
        grid: { containLabel: true, left: '2%', right: 110, bottom: '4%', top: '3%' },
        xAxis: { type: 'value', axisLabel: { formatter: v => (v / 1000) + ' tỷ', fontSize: 10 } },
        yAxis: { type: 'category', data: expData.map(d => d.name), axisLabel: { fontSize: 11 } },
        series: [{
            ...
            label: {
                ...
                formatter: p => {
                    const d = expData.find(x => x.value === p.value);
                    return (d.value / 1000) + ' tỷ | ' + d.change;
                }
            }
        }]
    }));
    ...
    // 5. Top Customers AR
    initOnce('rf-top-customers', () => ({
        animation: false,
        hoverLayerThreshold: -1,
        tooltip: { trigger: 'axis' },
        grid: { containLabel: true, left: '2%', right: '6%', bottom: '4%', top: '3%' },
        xAxis: { type: 'value', axisLabel: { formatter: '{value} tỷ', fontSize: 10 } },
        yAxis: { type: 'category', axisLabel: { fontSize: 11 },
            data: ['Delta Transport', 'Logistics Plus', 'Express Global', 'Swift Shipping', 'Ocean Freight', 'Direct Cargo'] },
        series: [{
            type: 'bar', barWidth: '55%',
            data: [3.2, 2.8, 2.1, 1.9, 1.5, 1.2],
            itemStyle: { color: colors.primary },
...
window.addEventListener('resize', () => {
    document.querySelectorAll('.chart, .rf-echart').forEach(el => {
